...
export namespace Fetch {
    ...
    export class Response {
        ...
        json<T>(): Promise<Result<T, JSONParseError | Error>> {
            return Result.promise<string, Error>(this.nativeResponse.text())
                .then(result => {
                    if (result.ok)
                        return JSON.parse<T>(result.value)
                            .mapError(error => new JSONParseError(error))
